...
stockProductos.forEach((producto) => {
    ...
    <p>${producto.desc}</p>
    <p>Tamaño: ${producto.talle}</p>
    <p class="precioProducto">Precio:$ ${producto.precio}</p>
    <p> <button id="agregar${producto.id}" class="boton-agregar">Agregar <i class="fas fa-shopping-cart"></i></button> </p>
...
const agregarAlCarrito = (prodId) => {
    ...
    //el carrito y se ve.
    actualizarCarrito(0) //LLAMAMOS A LA FUNCION QUE CREAMOS EN EL TERCER PASO. CADA VEZ Q SE 
    //MODIFICA EL CARRITO
}
...
const actualizarCarrito = () => {
    ...
    carrito.forEach((prod) => {
        ...
        // localStorage.setItem('nombre', prod.nombre)
        // localStorage.setItem('nombre', prod.cantidad)
        // let productname = localStorage.getItem('nombref', JSON.stringify(prod.nombre))
        // let quantity = localStorage.getItem('cantidadf', JSON.stringify(prod.cantidad))
        // let arrayDb = [productname, quantity]
        // localStorage.setItem('database', arrayDb)
    })
    ...
    //empezando en 0.
    Preciofinal= carrito.reduce((acc, prod) => acc + prod.cantidad * prod.precio, 0);
    localStorage.setItem('preciof', Preciofinal);
    obtener_localstorage();
    function obtener_localstorage(){
        if(localStorage.getItem('precio')){

            let preciof = json.parse(localStorage.getItem('precio'));
            console.log('precio');
        }else{
