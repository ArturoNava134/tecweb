...
class Productos extends DataBase {
    ...
    {
        $query = "SELECT * FROM products WHERE eliminado = 0";
    ...
    public function delete($id) {
        $query = "UPDATE products SET eliminado = 1 WHERE id = $id";
    ...
    public function search($search){
        $query = "SELECT * FROM products WHERE (id = '{$search}' OR nombre LIKE '%{$search}%' OR marca LIKE '%{$search}%' OR detalles LIKE '%{$search}%') AND eliminado = 0";
        $result = $this->conexion->query($query);
